...
Generator consists of 2 configuration macros at the start of `genpass.c` file: `MAX_CHAR`, `WORDS` and `MAX_WORD_LEN`.  
`MAX_CHAR` macro defines max amount of characters your password may consist of. Setting this macro to 0 disables character limit for your password. (0 by default)  
`WORDS` macro defines amount of words you want to be present in your generated password. (10 by default)  
`MAX_WORD_LEN` macro defines max amount of characters you want each word to contain in your password. (64 by default)    
...
2. Configure `MAX_CHAR`, `WORDS` and `MAX_WORD_LEN` macros in `genpass.c` based on your needs using your code editor.
